 ...
 * @abstract
 */
abstract class MB4WP_Integration {
	...
	public function __construct( $slug, array $options ) {
		...
		if( empty( $this->checkbox_name ) ) {
			$this->checkbox_name = '_mb4wp_subscribe_' . $this->slug;
		}
	}
	...
	protected function get_default_options() {
		...
		if( ! $defaults ) {
			$defaults = require MB4WP_PLUGIN_DIR . 'config/default-integration-options.php';
		}
	...
	protected function parse_options( array $options ) {
         ...
         * @ignore
		 */
		return (array) apply_filters( 'mb4wp_' . $slug . '_integration_options', $options );
	}
	...
	public function get_label_text() {
         ...
         * @ignore
		 */
		$label = (string) apply_filters( 'mb4wp_integration_checkbox_label', $label, $integration );
		return $label;
	}
	...
	protected function get_checkbox_attributes() {
         ...
         * @ignore
		 */
		$attributes = (array) apply_filters( 'mb4wp_integration_checkbox_attributes', $attributes, $integration );
         ...
         * @ignore
		 */
		$attributes = (array) apply_filters( 'mb4wp_integration_' . $slug . '_checkbox_attributes', $attributes, $integration );
	...
	public function get_checkbox_html() {
         ...
         */
        $show_checkbox = (bool) apply_filters( 'mb4wp_integration_show_checkbox', $show_checkbox, $integration_slug );
		...
		echo sprintf( '<!-- MailBlaze for WordPress v%s -->', MB4WP_VERSION );

        /** @ignore */
        do_action( 'mb4wp_integration_before_checkbox_wrapper', $this );

        /** @ignore */
		do_action( 'mb4wp_integration_'. $this->slug .'_before_checkbox_wrapper', $this );
        ...
        echo sprintf( '<%s class="mb4wp-checkbox mb4wp-checkbox-%s">', $wrapper_tag, esc_attr( $this->slug ) );
        echo '<label>';
        ...
        /** @ignore */
		do_action( 'mb4wp_integration_after_checkbox_wrapper', $this );

        /** @ignore */
		do_action( 'mb4wp_integration_'. $this->slug .'_after_checkbox_wrapper', $this );
		echo '<!-- / MailBlaze for WordPress -->';
	...
	/**
	 * Get the selected MailBlaze lists
	 *
	...
	public function get_lists() {
		...
		if( ! empty( $data['_mb4wp_lists'] ) ) {

			$lists = $data['_mb4wp_lists'];
		 ...
		 *
		 * @since 2.0
		 * @see MB4WP_Form::get_lists
		 * @ignore
		 */
		$lists = (array) apply_filters( 'mb4wp_lists', $lists );

		/**
		 * Filters the MailBlaze lists this integration should subscribe to
		 *
		 * @since 3.0
		 *
		 * @param array $lists
		 * @param MB4WP_Integration $integration
		 */
		$lists = (array) apply_filters( 'mb4wp_integration_lists', $lists, $integration );

		/**
		 * Filters the MailBlaze lists a specific integration should subscribe to
		 *
		 ...
		 * @param MB4WP_Integration $integration
		 */
		$lists = (array) apply_filters( 'mb4wp_integration_' . $slug . '_lists', $lists, $integration );
	...
	protected function subscribe( array $data, $related_object_id = 0 ) {
		...
		// validate lists
		if( empty( $list_ids ) ) {
			$log->warning( sprintf( '%s > No MailBlaze lists were selected', $this->name ) );
			return false;
		}
		 ...
		 * @param array $data
		 */
		$data = apply_filters( 'mb4wp_integration_data', $data );
		 ...
		 * @param int $related_object_id
		 */
		$data = apply_filters( "mb4wp_integration_{$slug}_data", $data, $related_object_id );
		 ...
		 * @ignore
		 */
		$data = apply_filters( 'mb4wp_integration_merge_vars', $data, $integration );

		/**
		 * @deprecated 4.0
		 * @ignore
		 */
		$data = apply_filters( "mb4wp_integration_{$slug}_merge_vars", $data, $integration );

		$email_type = mb4wp_get_email_type();

		$mapper = new MB4WP_List_Data_Mapper( $data, $list_ids );
		...
		foreach( $map as $list_id => $subscriber ) {
			...
			$subscriber->ip_signup = mb4wp_get_request_ip_address();

			/** @ignore (documented elsewhere) */
			$subscriber = apply_filters( 'mb4wp_subscriber_data', $subscriber );

			/**
			 * Filters subscriber data before it is sent to MailBlaze. Only fires for integration requests.
			 *
			 * @param MB4WP_MailBlaze_Subscriber $subscriber
			 */
			$subscriber = apply_filters( 'mb4wp_integration_subscriber_data', $subscriber );
			 ...
			 * @param int $related_object_id
			 */
			$subscriber = apply_filters( "mb4wp_integration_{$slug}_subscriber_data", $subscriber, $related_object_id );

			$result = $mailblaze->list_subscribe( $list_id, $subscriber->email_address, $subscriber->to_array(), $this->options['update_existing'], $this->options['replace_interests'] );
		}
		...
		if( ! $result ) {
			...
			if( $mailblaze->get_error_code() == 214 ) {
				...
				$log->error( sprintf( '%s > MailBlaze API Error: %s', $this->name, $mailblaze->get_error_message() ) );
			}
		 ...
		 * @param MB4WP_MailBlaze_Subscriber[] $subscriber_data
		 * @param int $related_object_id
		 */
		do_action( 'mb4wp_integration_subscribed', $integration, $subscriber->email_address, $subscriber->merge_fields, $map, $related_object_id );
	 ...
	 *
	 * @see MB4WP_Integration::$checkbox_name
	 * @see MB4WP_Integration::get_lists
	 * @see MB4WP_Integration::checkbox_was_checked
	 *
	 * @return array
	 */
