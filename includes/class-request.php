...
class MB4WP_Request {
	...
	public function __construct( $get = array(), $post = array(), $server = array() ) {
		$this->get = new MB4WP_Array_Bag( $get );
		$this->post = new MB4WP_Array_Bag( $post );
		$this->params = new MB4WP_Array_Bag( array_merge( $post, $get ) );
		$this->server = new MB4WP_Array_Bag( $server );
	}
