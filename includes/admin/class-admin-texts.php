...
class MB4WP_Admin_Texts {
	...
	public function footer_text( $text ) {
		...
		if(! empty( $_GET['page'] ) && strpos( $_GET['page'], 'mailblaze-for-wp' ) === 0 ) {
			$text = '';
		}
	...
	public function add_plugin_settings_link( $links, $file ) {
		...
		$settings_link = '<a href="' . admin_url( 'admin.php?page=mailblaze-for-wp' ) . '">'. __( 'Settings', 'mailblaze-for-wp' ) . '</a>';
	...
	public function add_plugin_meta_links( $links, $file ) {
         ...
         * @ignore
		 */
		$links = (array) apply_filters( 'mb4wp_admin_plugin_meta_links', $links );
