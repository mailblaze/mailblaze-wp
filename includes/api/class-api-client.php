...
class MB4WP_API_Client {
  ...
  * @var string
  */
  private $api_url = 'https://control.mailblaze.com/api/';
  ...
  */
  private function request( $method, $resource, array $data = array() ) {
    ...
    if( empty( $this->api_key ) ) {
      throw new MB4WP_API_Exception( "Missing API key", 001 );
    }
    ...
    $args = array(
      ...
      'timeout' => 10,
      'sslverify' => apply_filters( 'mb4wp_use_sslverify', true ),
    );
    ...
    * @param array $args
    */
    $args = apply_filters( 'mb4wp_http_request_args', $args, $url );
  ...
  */
  private function get_headers() {
    ...
    $headers['User-Agent'] = 'mb4wp/' . MB4WP_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' );
  ...
  */
  private function parse_response( $response ) {
    ...
    if( $response instanceof WP_Error ) {
      throw new MB4WP_API_Connection_Exception( $response->get_error_message(), (int) $response->get_error_code() );
    }
    ...
    // set body to "true" in case MailBlaze returned No Content
    ...
    if( $code >= 400 ) {
      ...
      if( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
        throw new MB4WP_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
      }

      if( $code === 404 ) {
        throw new MB4WP_API_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
      }

      // mailblaze returned an error..
      throw new MB4WP_API_Exception( $message, $code, $this->last_request, $this->last_response, $data );
    }
    ...
    throw new MB4WP_API_Exception( $message, $code, $this->last_request, $this->last_response );
  }
