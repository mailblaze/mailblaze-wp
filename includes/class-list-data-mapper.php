...
/**
* Class MB4WP_Field_Map
*
* @access private
* @since 4.0
* @ignore
*/
class MB4WP_List_Data_Mapper {
	...
	*/
	public function __construct( array $data, array $list_ids ) {
		...
		$this->formatter = new MB4WP_Field_Formatter();
	...
	/**
	* @return MB4WP_MailBlaze_Subscriber[]
	*/
	public function map() {
		...
		foreach( $this->list_ids as $list_id ) {
			...
			if( $list instanceof MB4WP_MailBlaze_List ) {
	...
	/**
	* @param MB4WP_MailBlaze_List $list
	*
	* @return MB4WP_MailBlaze_Subscriber
	*/
	protected function map_list( MB4WP_MailBlaze_List $list ) {

		$subscriber = new MB4WP_MailBlaze_Subscriber();
		...
		// find language
		/* @see http://kb.mailblaze.com/lists/managing-subscribers/view-and-edit-subscriber-languages?utm_source=mc-api&utm_medium=docs&utm_campaign=apidocs&_ga=1.211519638.2083589671.1469697070 */
	...
	*/
	private function format_merge_field_value( $field_value, $field_type ) {
		...
		*
		* Use this to format a field value according to the field type (in MailBlaze).
		*
		* @since 3.0
		* @param string $field_value The value
		* @param string $field_type The type of the field (in MailBlaze)
		*/
		$field_value = apply_filters( 'mb4wp_format_field_value', $field_value, $field_type );
