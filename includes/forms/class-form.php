 ...
 *
 * To get a form instance, use `mb4wp_get_form( $id );` where `$id` is the post ID.
 *
 * @access public
 * @since 3.0
 */
class MB4WP_Form {
    ...
    public static function get_instance( $post = 0 ) {
        ...
        if( $post instanceof WP_Post ) {
            ...
            if( empty( $post_id ) ) {
                $post_id = (int) get_option( 'mb4wp_default_form_id', 0 );
            }
        }
        ...
        $form = new MB4WP_Form( $post_id, $post, $post_meta );
    ...
    */
     ...
     *
     * @see MB4WP_Form_Element::get_response_html()
     *
     * @return string
     */
     ...
     * @param array $config
     * @return MB4WP_Form_element
     */
    public function get_element( $element_id = 'mb4wp-form', $config = array() ) {
        return new MB4WP_Form_Element( $this, $element_id, $config );
    }

    /**
     * Get HTML string for this form.
     *
     * If you want to output a form, use `mb4wp_show_form` instead as it.
     *
     * @param string $element_id
     * @param array $config
     *
     * @return string
     */
    public function get_html( $element_id = 'mb4wp-form', array $config = array() ) {
    ...
    protected function load_settings( $post_meta = array() ) {
        ...
        if( ! $default_settings ) {
            $default_settings = include MB4WP_PLUGIN_DIR . 'config/default-form-settings.php';
        }
        ...
        if( ! empty( $post_meta['_mb4wp_settings'] ) ) {
            $meta = $post_meta['_mb4wp_settings'][0];
         ...
         */
        $settings = (array) apply_filters( 'mb4wp_form_settings', $settings, $form );
    ...
    protected function load_messages( $post_meta = array() ) {
        ...
        $default_messages = include MB4WP_PLUGIN_DIR . 'config/default-form-messages.php';
         ...
         * @param MB4WP_Form $form
         */
        $messages = (array) apply_filters( 'mb4wp_form_messages', $messages, $form );
    ...
    */
    public function add_notice( $text, $type = 'notice' ) {
        $this->notices[] = new MB4WP_Form_Notice( $text, $type );
    }
    ...
    public function __toString() {
        return mb4wp_show_form( $this->ID, array(), false );
    }
    ...
    public function get_redirect_url() {
         ...
         * @param MB4WP_Form $form
         */
        $url = (string) apply_filters( 'mb4wp_form_redirect_url', $url, $form );
        return $url;
    }
    ...
    public function validate() {
        ...
        if( ! isset( $this->raw_data['_mb4wp_timestamp'] ) || $this->raw_data['_mb4wp_timestamp'] > ( time() - 2 ) ) {
            $errors[] = 'spam';
        } else if( ! isset( $this->raw_data['_mb4wp_honeypot'] ) || ! empty( $this->raw_data['_mb4wp_honeypot'] ) ) {
            $errors[] = 'spam';
        }
        ...
        if( empty( $errors ) ) {
            ...
            foreach( $this->get_required_fields() as $field ) {
                $value = mb4wp_array_get( $this->data, $field );
         ...
         * @param MB4WP_Form $form
         */
        $errors = (array) apply_filters( 'mb4wp_form_errors', $errors, $form );

        /**
         * @ignore
         * @deprecated 3.0 Use `mb4wp_form_errors` instead
         */
        $form_validity = apply_filters( 'mb4wp_valid_form_request', true, $this->data );
    ...
    public function handle_request( array $data ) {
        ...
        $map = array(
            '_mb4wp_lists' => 'lists',
            '_mb4wp_action' => 'action',
            '_mb4wp_form_element_id' => 'element_id',
            '_mb4wp_email_type' => 'email_type'
        );
    ...
    protected function parse_request_data( array $data ) {
         ...
         * @param MB4WP_Form $form The form instance.
         */
        $ignored_field_names = apply_filters( 'mb4wp_form_ignored_field_names', $ignored_field_names, $form );
    ...
    /**
     * Get MailBlaze lists this form subscribes to
     *
     * @return array
     */
    public function get_lists() {
        ...
        /**
         * Filters MailBlaze lists new subscribers should be added to.
         *
         * @param array $lists
         */
        ...
        /**
         * Filters MailBlaze lists new subscribers coming from this form should be added to.
         *
         * @param array $lists
         * @param MB4WP_Form $form
         */
        $lists = (array) apply_filters( 'mb4wp_form_lists', $lists, $form );
    ...
    public function get_data() {
         ...
         * @param MB4WP_Form $form
         */
        $data = apply_filters( 'mb4wp_form_data', $data, $form );
    ...
    public function get_required_fields() {
        ...
        // workaround for #261 (https://github.com/limabean/mailblaze-for-wordpress/issues/261)
        ...
        // fix uppercased subkeys, see https://github.com/limabean/mailblaze-for-wordpress/issues/516
         ...
         *
         * - All fields which are required for the selected MailBlaze lists
         * - All fields in the form with a `required` attribute.
         *
         * @param array $required_fields
         * @param MB4WP_Form $form
         */
        $required_fields = (array) apply_filters( 'mb4wp_form_required_fields', $required_fields, $form );

        return $required_fields;
    }

    /**
     * Get "email_type" setting for new MailBlaze subscribers added by this form.
     *
     * @return string
     */
    ...
    */
    public function get_message( $key ) {
        ...
        if( $key === 'no_lists_selected' && current_user_can( 'manage_options' ) ) {
            $message .= sprintf( ' (<a href="%s">%s</a>)', mb4wp_get_edit_form_url( $this->ID, 'settings' ), 'edit form settings' );
        }
