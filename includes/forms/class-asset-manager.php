 ...
 * @ignore
*/
class MB4WP_Form_Asset_Manager {
	...
	/**
	 * MB4WP_Form_Asset_Manager constructor.
	 */
	...
	public function hook() {
		...
		add_action( 'mb4wp_output_form', array( $this, 'before_output_form' ) );
	...
	public function register_assets() {
		 ...
		 * @ignore
		 */
		do_action( 'mb4wp_register_form_assets', $suffix );
	}
	...
	public function get_stylesheet_url( $stylesheet ) {
		...
		$suffix = $this->filename_suffix;
		return MB4WP_PLUGIN_URL . 'assets/css/form-' . $stylesheet . $suffix . '.css';
	}
	...
	public function get_active_stylesheets() {
		$stylesheets = (array) get_option( 'mb4wp_form_stylesheets', array() );
		 ...
		 * Each value is prefixed with `mb4wp-form-` to get the handle.
		 *
		 ...
		 */
		$stylesheets = (array) apply_filters( 'mb4wp_form_stylesheets', $stylesheets );
		return $stylesheets;
	}
	...
	public function load_stylesheets( ) {
		...
		foreach( $stylesheets as $stylesheet ) {
			...
			$handle = 'mb4wp-form-' . $stylesheet;
			wp_enqueue_style( $handle, $this->get_stylesheet_url( $stylesheet ), array(), MB4WP_VERSION );
		 ...
		 * @ignore
		 */
		do_action( 'mb4wp_load_form_stylesheets', $stylesheets );
	...
	public function get_javascript_config() {
		...
		$submitted_form = mb4wp_get_submitted_form();
		 ...
		 * @since 3.0
		 */
		$config['auto_scroll'] = apply_filters( 'mb4wp_form_auto_scroll', $auto_scroll );
	...
	/**
	 * Prints dummy JavaScript which allows people to call `mb4wp.forms.on()` before the JS is loaded.
	 */
	...
	*/
	public function load_scripts() {
		...
		/** @ignore */
		$load_scripts = apply_filters( 'mb4wp_load_form_scripts', $load_scripts );
		if( ! $load_scripts ) {
			return;
		}
		...
		wp_enqueue_script( 'mb4wp-forms-api', MB4WP_PLUGIN_URL . 'assets/js/forms-api'.  $this->filename_suffix .'.js', array(), MB4WP_VERSION, true );
		wp_localize_script( 'mb4wp-forms-api', 'mb4wp_forms_config', $this->get_javascript_config() );

		// load placeholder polyfill if browser is Internet Explorer
		wp_enqueue_script( 'mb4wp-forms-placeholders', MB4WP_PLUGIN_URL . 'assets/js/third-party/placeholders.min.js', array(), MB4WP_VERSION, true );
		$wp_scripts->add_data( 'mb4wp-forms-placeholders', 'conditional', 'lte IE 9' );
		...
		/** @ignore */
		do_action( 'mb4wp_load_form_scripts' );
	}
