 ...
 */
function mb4wp($service = null)
{
    static $mb4wp;

    if (!$mb4wp) {
        $mb4wp = new MB4WP_Container();
    }

    if ($service) {
        return $mb4wp->get($service);
    }
...
/**
 * Gets the MailBlaze for WP options from the database
 ...
 * @return array
 */
function mb4wp_get_options()
{
    static $options;

    if (!$options) {
        $defaults = require MB4WP_PLUGIN_DIR . 'config/default-settings.php';
        $options = (array)get_option('mb4wp', array());
        $options = array_merge($defaults, $options);
    }

    /**
     * Filters the MailBlaze for WordPress settings (general).
     *
     * @param array $options
     */
    return apply_filters('mb4wp_settings', $options);
}

/**
 * Gets the MailBlaze for WP API class and injects it with the API key
 *
 *
 * @access public
 *
 * @return MB4WP_API
 */
function mb4wp_get_api()
{
    $opts = mb4wp_get_options();
    $instance = new MB4WP_API($opts['api_key']);
    return $instance;
}
 ...
 */
function mb4wp_get_debug_log()
{
    $opts = mb4wp_get_options();

    // get default log file location
    $upload_dir = wp_upload_dir(null, false);
    $file = trailingslashit($upload_dir['basedir']) . 'mb4wp-debug-log.php';

    /**
     * Filters the log file to write to.
     *
     * @param string $file The log file location. Default: /wp-content/uploads/mb4wp-debug.log
     */
    $file = apply_filters('mb4wp_debug_log_file', $file);
     ...
     */
    $level = apply_filters('mb4wp_debug_log_level', $opts['debug_log_level']);

    return new MB4WP_Debug_Log($file, $level);
}
 ...
 * @return string
 */
function mb4wp_get_request_url() {
     global $wp;
 ...
 * @return string
 */
function mb4wp_get_request_path() 
{
...
* @return string
*/
function mb4wp_get_request_ip_address() 
{
 ...
 * @return mixed
 */
function mb4wp_sanitize_deep($value)
{
    ...
    if (is_scalar($value)) {
        ...
        $value = array_map('mb4wp_sanitize_deep', $value);
    } elseif (is_object($value)) {
        $vars = get_object_vars($value);
        foreach ($vars as $key => $data) {
            $value->{$key} = mb4wp_sanitize_deep($data);
        }
    }
 ...
 * @return array
 */
function _mb4wp_update_groupings_data($data = array())
{
    ...
    $map = get_option('mb4wp_groupings_map', array());
 ...
 * @return array
 */
function mb4wp_add_name_data($data = array())
{
...
function mb4wp_get_email_type()
{
     ...
     */
    $email_type = (string)apply_filters('mb4wp_email_type', $email_type);
 ...
 * @return string
 */
function mb4wp_obfuscate_string($string)
{
 ...
 * @ignore
 */
function _mb4wp_obfuscate_email_addresses_callback($m)
{
 ...
 * @return string
 */
function mb4wp_obfuscate_email_addresses($string)
{
    return preg_replace_callback('/([\w\.]{1,4})([\w\.]*)\@(\w{1,2})(\w*)\.(\w+)/', '_mb4wp_obfuscate_email_addresses_callback', $string);
}

/**
 * Refreshes MailBlaze lists. This can take a while if the connected MailBlaze account has many lists.
 *
 * @return void
 */
function mb4wp_refresh_mailblaze_lists()
{
    $mailblaze = new MB4WP_MailBlaze();
    $mailblaze->fetch_lists();
}
...
* @return mixed
*/
function mb4wp_array_get( $array, $key, $default = null ) {
    if ( is_null( $key ) ) {
        return $array;
    }
