...
/**
 * Class MB4WP_Contact_Form_7_Integration
 *
 * @ignore
 */
class MB4WP_Contact_Form_7_Integration extends MB4WP_Integration {
	...
	*/
	public function init() {
		...
		if ( function_exists( 'wpcf7_add_form_tag' ) ) {
			wpcf7_add_form_tag( 'mb4wp_checkbox', array( $this, 'shortcode' ) );
		} else {
			wpcf7_add_shortcode( 'mb4wp_checkbox', array( $this, 'shortcode' ) );
		}
	 ...
	 *
	 * - _mb4wp_subscribe_contact-form-7
	 * - mb4wp-subscribe
	 *
	 * @return bool
	 */
	public function checkbox_was_checked() {
		...
		return ( isset( $data[ $this->checkbox_name ] ) && $data[ $this->checkbox_name ] == 1 )
			|| ( isset( $data[ 'mb4wp-subscribe' ] ) && $data[ 'mb4wp-subscribe' ] == 1 );
	}

	/**
	* Alter Contact Form 7 data.
	*
	* Adds mb4wp_checkbox to post data so users can use `mb4wp_checkbox` in their email templates
	*
	* @param array $data
	* @return array
	*/
	public function alter_cf7_data( $data = array() ) {
		$data['mb4wp_checkbox'] = $this->checkbox_was_checked() ? __( 'Yes' ) : __( 'No' );
		return $data;
	}
	 ...
	 *
	 * @todo improve smart guessing based on selected MailBlaze lists
	 *
	...
	public function process( $cf7_form ) {
		...
		$parser = new MB4WP_Field_Guesser( $this->get_data() );
