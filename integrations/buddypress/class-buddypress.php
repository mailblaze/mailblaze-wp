...
/**
 * Class MB4WP_BuddyPress_Integration
 *
 * @ignore
 */
class MB4WP_BuddyPress_Integration extends MB4WP_User_Integration {
	...
	public function add_hooks() {
		...
		if ( is_multisite() ) {
			 ...
			 * activation process.
			 *
			 * To avoid all signups being subscribed to the MailBlaze list until they
		 ...
		 * plugins to delay the MailBlaze signup process.
		 *
		 * Plugins can hook into the 'mb4wp_integration_buddypress_should_subscribe' filter to prevent
		 * subscriptions from taking place:
		 *
		 * add_filter( 'mb4wp_integration_buddypress_should_subscribe', '__return_false' );
         *
		 * The plugin would then then call:
		 *
		 * do_action( 'mb4wp_integration_buddypress_subscribe_user', $user_id );
		 *
		 * to perform the subscription at a later point.
		 */
		add_action( 'mb4wp_integration_buddypress_subscribe_user', array( $this, 'subscribe_buddypress_user' ), 10, 1 );
	...
	public function subscribe_from_form( $user_id, $user_login, $user_password, $user_email ) {
		...
		/**
		 * Allow other plugins to prevent the MailBlaze sign-up.
		 *
		 * @param bool $subscribe False does not subscribe the user.
		 * @param int $user_id The user ID to subscribe
		 */
        $subscribe = apply_filters( 'mb4wp_integration_buddypress_should_subscribe', $subscribe, $user_id );
	...
	public function subscribe_from_usermeta( $user_id, $key, $userdata ) {
        ...
        /**
         * @ignore Documented elsewhere, see MB4WP_BuddyPress_Integration::subscribe_from_form.
         */
        $subscribe = apply_filters( 'mb4wp_integration_buddypress_should_subscribe', $subscribe, $user_id );
        if( ! $subscribe ) {
            return false;
	...
	/**
	 * Subscribes a user to MailBlaze list(s).
	 *
	 * @param int $user_id The user ID to subscribe
     * @return bool
	 */
