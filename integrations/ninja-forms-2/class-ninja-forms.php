...
/**
 * Class MB4WP_Ninja_Forms_v2_Integration
 *
 * @ignore
 */
class MB4WP_Ninja_Forms_v2_Integration extends MB4WP_Integration {
	...
	public function register_field() {
		...
		ninja_forms_register_field( 'mb4wp-subscribe', $args );
	}
	...
	public function process( $id, $value ) {
		...
		foreach( $fields as $field_id => $field_value ) {
			...
			// try admin label for "mb4wp-" prefixed fields, otherwise use general label
			$label = $ninja_forms_processing->get_field_setting( $field_id, 'admin_label' );
			if( empty( $label ) || stripos( $label, 'mb4wp-' ) !== 0 ) {
		...
		// guess mailblaze variables
		$parser = new MB4WP_Field_Guesser( $pretty );
