...
var FieldFactory = function FieldFactory(fields, i18n) {
    ...
    /**
     * Normalizes the field type which is passed by MailBlaze
     *
     * @param type
     * @returns {*}
     */
    ...
    function registerMergeField(mergeField) {
        ...
        if (data.type !== 'address') {
            ...
            register(category, { name: data.name + '[addr1]', type: 'text', mailblazeType: 'address', title: i18n.streetAddress });
            register(category, { name: data.name + '[city]', type: 'text', mailblazeType: 'address', title: i18n.city });
            register(category, { name: data.name + '[state]', type: 'text', mailblazeType: 'address', title: i18n.state });
            register(category, { name: data.name + '[zip]', type: 'text', mailblazeType: 'address', title: i18n.zip });
            register(category, { name: data.name + '[country]', type: 'select', mailblazeType: 'address', title: i18n.country, choices: mb4wp_vars.countries });
        }
    ...
    /**
     * Register a field for a MailBlaze grouping
     *
...
module.exports = function (m, events) {
    ...
    var Field = function Field(data) {
        ...
        this.mailblazeType = prop(data.mailblazeType || '');
...
var domDirty = false;
var editor;
var element = document.getElementById('mb4wp-form-content');
var previewFrame = document.getElementById('mb4wp-form-preview');
var previewDom;
var templateRegex = /\{[^{}]+\}/g;
...
function setPreviewDom() {
    ...
    previewDom = frameContent.querySelector('.mb4wp-form-fields');
...
function updatePreview() {
    ...
    previewDom.dispatchEvent(new Event('mb4wp-refresh'));
}
...
var FormWatcher = function FormWatcher(m, editor, settings, fields, events, helpers) {
    ...
    function updateFields() {
        fields.getAll().forEach(function (field) {
            ...
            if (field.mailblazeType() === 'address') {
    ...
    function findRequiredFields() {
        ...
        // query fields required by MailBlaze
...
function render() {
    ...
    var container = document.querySelector('.mb4wp-notices');
    if (!container) {
        container = document.createElement('div');
        container.className = 'mb4wp-notices';
...
function init(editor, fields) {
    ...
    var groupingsNotice = function groupingsNotice() {
        var text = "Your form contains old style <code>GROUPINGS</code> fields. <br /><br />Please remove these fields from your form and then re-add them through the available field buttons to make sure your data is getting through to MailBlaze correctly.";
    ...
    var requiredFieldsNotice = function requiredFieldsNotice() {
        ...
        var text = '<strong>Heads up!</strong> Your form is missing list fields that are required in MailBlaze. Either add these fields to your form or mark them as optional in MailBlaze.';
...
var i18n = window.mb4wp_forms_i18n;
var m = window.mb4wp.deps.mithril;
var events = mb4wp.events;
var settings = mb4wp.settings;
var helpers = mb4wp.helpers;
...
m.mount(document.getElementById('mb4wp-field-wizard'), fieldHelper);
...
fieldsFactory.registerCustomFields(mb4wp_vars.mailblaze.lists);
...
// expose some methods
window.mb4wp = window.mb4wp || {};
window.mb4wp.forms = window.mb4wp.forms || {};
window.mb4wp.forms.editor = editor;
window.mb4wp.forms.fields = fields;
